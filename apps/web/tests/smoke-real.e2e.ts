...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () =>
        document.title !== 'DeepSeek Harness Workbench' &&
        document.title.endsWith(' — DeepSeek Harness Workbench'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — DeepSeek Harness Workbench`,
      durableTitle,
      { timeout: 15_000 },
    )
