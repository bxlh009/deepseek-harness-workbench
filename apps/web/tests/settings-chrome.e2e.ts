  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    const readState = async (target: Page = page): Promise<ThemeState> => await target.evaluate(() => {
      ...
      return {
        ...
        skin: document.body.getAttribute('data-ds-skin'),
    ...
    expect(light.skin).toBe('classic')
    ...
    expect(reloaded.skin).toBe('classic')
    ...
    try {
      ...
      expect(secondState.skin).toBe('classic')
    ...
    await expect.poll(async () => (await readState()).attr, { timeout: 5_000 }).toBe(false)
    expectThemeColorSynchronized(await readState())
    const appearanceDialog = page.getByRole('dialog', { name: '设置' })
    const oceanSkin = appearanceDialog.getByRole('button', { name: '海洋' })
    await oceanSkin.click()
    await expect.poll(() => oceanSkin.getAttribute('aria-pressed'), { timeout: 5_000 }).toBe('true')
    expect((await readState()).skin).toBe('ocean')
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/ui-theme:[\s\S]*skin: ocean/)
    await appearanceDialog.getByRole('button', { name: '经典' }).click()
    await expect.poll(async () => (await readState()).skin, { timeout: 5_000 }).toBe('classic')
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  }, 90_000)
