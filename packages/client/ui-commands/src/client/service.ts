...
// Type-only: allows the optional command-copy lookup to follow the active UI locale.
...
/** Host command descriptions that are stable enough to receive product copy. */
const LOCALIZED_HOST_DESCRIPTIONS: Record<string, { source: string; key: string }> = {
  compact: { source: 'Compact older conversation history', key: 'command.compact.description' },
  export: { source: 'Download this Session log as a ZIP archive', key: 'command.export.description' },
  feedback: { source: 'record feedback about this session', key: 'command.feedback.description' },
  goal: { source: 'set or view the goal for a long-running task', key: 'command.goal.description' },
  permission: { source: 'Switch the permission preset (sandbox mode + approval policy)', key: 'command.permission.description' },
  plan: { source: 'Enter or leave plan mode', key: 'command.plan.description' },
}
...
export class CommandUiRuntime extends Service implements CommandUiContract {
  ...
  private readonly translateCommand: ((key: string) => string) | undefined
  ...
  constructor(ctx: Context) {
    super(ctx, 'commandUi')
    const locale = ctx.get('locale') as { bind(ns: string): (key: string) => string } | undefined
    this.translateCommand = locale?.bind('command')
  ...
  /** Keep host behavior and slash names intact while translating stable product descriptions. */
  private commandDescription(name: string, description: string): string {
    const copy = LOCALIZED_HOST_DESCRIPTIONS[name]
    if (copy === undefined || copy.source !== description || this.translateCommand === undefined) return description
    return this.translateCommand(copy.key)
  }
  ...
  private async candidates(session: ClientSessionContext, req: CandidateRequest): Promise<readonly InputTriggerCandidate[]> {
    ...
    for (const c of list) {
      ...
      rows.push({
        name: c.name,
        description: this.commandDescription(c.name, c.description),
