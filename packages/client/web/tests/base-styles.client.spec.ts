 ...
 * the shell-owned image-skin sheet is present, and scrollbar.css follows
 * design-platform.css because it reads that sheet's tokens.
 */
...
/** Resolve a shell-owned relative import from the base sheet. */
function resolveLocalSheet(specifier: string): string {
  return fileURLToPath(new URL(`../src/${specifier.slice(2)}`, import.meta.url))
}
...
describe('web shell base.css', () => {
  ...
  it('imports every sheet from the theme package and each one exists', () => {
    ...
    const themeImports = imports.filter(specifier => specifier.startsWith(`${THEME_PACKAGE}/styles/`))
    const localImports = imports.filter(specifier => !specifier.startsWith(`${THEME_PACKAGE}/styles/`))
    ...
    for (const specifier of localImports) {
      ...
      expect(existsSync(resolveLocalSheet(specifier)), specifier).toBe(true)
    }
  })
