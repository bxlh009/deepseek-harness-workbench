...
export function apply(ctx: Context): void {
  ...
  // Register synchronously during apply. AppRoot flips its settled signal as
  // soon as the loader sweep completes; nesting this root registration inside
  // another effect leaves a small but real window where the sweep can observe
  // an active app-shell fiber while the root face is not yet readable.
  const disposeService = ctx.root.reflect.provide('appShell', {
    renderApp: (): ReactNode => {
      renderApp ??= buildRenderApp({ ctx })
      return renderApp()
    },
  })
  ctx.effect(() => () => { void disposeService() }, 'app-shell: root service cleanup')
}
