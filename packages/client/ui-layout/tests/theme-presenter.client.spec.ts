...
import type { ThemeSkin, ThemeSnapshot } from '@deepseek-ai/dsh-client-ui-theme/client'
import {
  BOOTSTRAP_BACKGROUND_ATTRIBUTE, BOOTSTRAP_TOKENS_ATTRIBUTE, DARK_ATTRIBUTE, SKIN_ATTRIBUTE, ThemePresenter,
...
function snapshot(
  ...
  return { preference: colorScheme, skin, active, themes: [active], skins: [], revision: 1 }
}
...
describe('ThemePresenter', () => {
  ...
  it('dark scheme sets root color-scheme, the attribute, and metadata; switching to light updates one node', () => {
    ...
    presenter.apply(snapshot('light', {}, 'ocean'))
    expect(document.documentElement.style.colorScheme).toBe('light')
    expect(document.body.hasAttribute(DARK_ATTRIBUTE)).toBe(false)
  ...
  it('retracts skin tokens written by the host bootstrap on first client apply', () => {
    document.body.style.setProperty('--dsw-alias-brand-primary', 'rgb(96, 165, 250)')
    document.body.setAttribute(BOOTSTRAP_TOKENS_ATTRIBUTE, '--dsw-alias-brand-primary')
    const presenter = new ThemePresenter()
    presenter.apply(snapshot('light'))
