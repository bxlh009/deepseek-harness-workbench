...
已配置的融合方案还会获得一个盲测模型竞技场。每轮把同一条隔离且无工具的题目并发发送给全部候选路由与融合路由，先将回复匿名标为 A–E；用户投票后才揭示路由、耗时、token 数和累计胜场。胜场只存在组件状态中，页面刷新即清空；竞技场不会写入对话 Session。

同一区域还可配置一个全局视觉代理。图片到达未声明图片输入能力的模型时，`llm-fusion` 流中间件会把不同附件各发送一次给已确认的视觉路由，再用文字标记替换图片块，并附带有长度上限的视觉报告调用原路由。声明原生图片输入能力的模型会跳过该代理。用户明确确认图片字节会离开 Harness 并发送给所选提供方之前，这项设置无法启用。
...
提供方删除开始前，页面会检查生效的 `llm-fusion` 配置。任何候选、汇总或视觉代理引用都会阻止删除并点名依赖配置；移除这些依赖前，不会尝试修改凭据或设置。
...
桌面版还提供内置的 **FreeLLMAPI** OpenAI 兼容网关预设，地址固定为 `http://127.0.0.1:31415/v1`。选择它后会出现「**管理 FreeLLMAPI**」按钮，由桌面外壳打开私有管理界面；用户在其中添加自己的上游免费额度提供方密钥，把生成的统一 API 密钥复制到这张提供方卡片，再获取该账户实际可用的模型。纯浏览器版不显示管理按钮，也不会自行启动网关。
...
竞技场会通过独立的 `llm.arena` 调用向每条待比较路由发送一条相同的用户消息。界面会立即为每个候选模型和融合模型显示一个匿名卡片，并在对应路由完成时逐张填入；请求不带工具或对话历史，普通设置编辑不会发送模型请求。

#### KV Cache 影响

每个竞技场回答都是仅含一条消息的冷请求，因此没有可复用的对话前缀，也没有跨路由 KV cache 收益。
