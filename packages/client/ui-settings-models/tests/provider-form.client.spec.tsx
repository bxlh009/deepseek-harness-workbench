...
describe('model list editing', () => {
  ...
  it('lets one declared model opt into image input without changing its siblings', async () => {
    ...
    expect(screen.getByLabelText(`${en.modelVisionInput} 1`)).toBeTruthy()
...
describe('hand-declared providers', () => {
  ...
  it('prefills an Ollama route and uses real model discovery as its connection check', async () => {
    const { discover } = mountCard()

    fireEvent.click(screen.getByRole('button', { name: 'Ollama' }))

    expect(screen.getByLabelText<HTMLInputElement>(en.customRoute).value).toBe('ollama')
    expect(screen.getByLabelText<HTMLInputElement>(en.customDisplayName).value).toBe('Ollama')
    expect(screen.getByLabelText<HTMLInputElement>(en.baseUrl).value).toBe('http://127.0.0.1:11434/v1')
