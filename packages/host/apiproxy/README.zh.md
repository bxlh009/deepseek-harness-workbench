...
`llm.arena` 是唯一直接执行模型的方法：它向一至五条路由发送一条隔离的用户消息，不带工具或 Session 历史，并返回可见文本、耗时、token 用量和逐路由失败。设置页竞技场使用独立的单路调用，让匿名答案卡片逐张完成。该包的其他方法不会组装模型请求。

#### KV Cache 影响

竞技场调用不共享对话前缀；每条待比较路由收到的都是仅含一条消息的冷请求。
